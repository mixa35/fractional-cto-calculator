...
prommer.net's advisory side has long guides and a single "Schedule Discovery Call" button, but nothing interactive between reading and booking, while the training side already runs 15+ calculators. I built a Fractional CTO vs Full-Time CTO cost calculator to close that gap: a visitor sees full-time cost compared against the site's published fractional tiers (10–15 hrs/week Strategic Advisory, 15–20 hrs/week Active Leadership), gets a suggested best fit, and lands on the existing Schedule Discovery Call button. Every number comes from prommer.net's published $400K+ full-time figure and hours ranges, or from the visitor's own inputs. A free OpenRouter model rewrites the summary through a Netlify function, with a built-in summary as the always-on baseline so the page never breaks. Built AI-first with Claude Code in one session, with CLAUDE.md as the shared brief.
